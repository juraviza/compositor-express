...
@Injectable()
export class EmotionalPromptGeneratorService {
  private readonly logger = new Logger(EmotionalPromptGeneratorService.name);

  constructor(private readonly llm: LlmService) {}

  /**
   * Analyze lyrics and generate emotionally-aligned video prompts.
   * Creates multiple variations for video generation.
   */
  async generateVideoPrompts(
    title: string,
    lyrics: string,
    artistName: string = 'Juan Rafael',
  ): Promise<EmotionalPromptResult> {
...
Output JSON with: emotion, visualStyle, musicSentiment, prompts (as array of cinema-grade prompts).`;
    ...
    try {
      const data = await this.llm.chatJson<EmotionalPromptResult>(sys, user, 30000);
  ...
  /**
   * Fallback prompts when LLM fails.
   */
  private getFallbackPrompts(title: string, lyrics: string, artistName: string): EmotionalPromptResult {
